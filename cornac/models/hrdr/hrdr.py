...
class Model:
    def __init__(self, n_users, n_items, vocab, global_mean,
        ...
        embedding_matrix[:4, :] = np.zeros((4, self.embedding_size), dtype=np.float32)
        ...
        l_user_mlp = keras.models.Sequential([
            ...
            layers.Dense(self.n_factors, activation="relu"),
        ...
        a_user = layers.Dense(1, activation=None, use_bias=True)(
            layers.Dense(self.attention_size, activation="relu", use_bias=True)(
                tf.multiply(
                    user_review_h,
                    tf.expand_dims(layers.Dense(self.n_filters, use_bias=True)(user_rating_h), 1)
                )
            )
        )
        ...
        a_item = layers.Dense(1, activation=None, use_bias=True)(
            layers.Dense(self.attention_size, activation="relu", use_bias=True)(
                tf.multiply(
                    item_review_h,
                    tf.expand_dims(layers.Dense(self.n_filters, use_bias=True)(item_rating_h), 1)
                )
            )
        )
    ...
    def get_weights(self, train_set, batch_size=64, max_num_review=32):
        ...
        P = np.zeros((self.n_users, self.n_factors + self.n_factors + self.id_embedding_size), dtype=np.float32)
        Q = np.zeros((self.n_items, self.n_factors + self.n_factors + self.id_embedding_size), dtype=np.float32)
        ...
        for batch_users in train_set.user_iter(batch_size):
            ...
            P[batch_users] = pu.numpy().reshape(len(batch_users), self.n_factors + self.n_factors + self.id_embedding_size)
        for batch_items in train_set.item_iter(batch_size):
            ...
            Q[batch_items] = qi.numpy().reshape(len(batch_items), self.n_factors + self.n_factors + self.id_embedding_size)
